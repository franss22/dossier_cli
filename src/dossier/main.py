...
from dossier.utils.dir import create_run_directory
...
def process_recording(
    ...
    chunk_minutes = (
        ...
        else config.audio.chunk_minutes
    )
    overlap_seconds = (
        overlap_seconds
        if overlap_seconds is not None
        else config.audio.overlap_seconds
    )
    ...
    for stream in streams:
        typer.echo(f"  Index: {stream['index']}, Codec: {stream['codec_name']}, "
